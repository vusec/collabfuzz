...
#[no_mangle]
pub extern "C" fn __inst_count_create() {
    ...
    *tracer_opt = Some(Tracer::new(output_path, enable_output));
    *tainter_opt = Some(Tainter::new(input_path_opt, enable_debug));
}

#[no_mangle]
pub extern "C" fn __inst_count_destroy() {
    let mut tracer_opt = TRACER.lock().unwrap();
    let mut tainter_opt = TAINTER.lock().unwrap();
...
#[no_mangle]
pub extern "C" fn __icountw___inst_count_trace(
    id: IDType,
    _traced_value: u64,
    _id_shadow: ShadowType,
    ...
    let tracer = tracer_opt.as_mut().unwrap();
    tracer.trace_terminator_taint(id, traced_value_shadow);
}
...
#[no_mangle]
pub extern "C" fn tainter_get_init_count() -> ShadowType {
    let tainter_opt = TAINTER.lock().unwrap();
    if tainter_opt.is_none() {
        // Be safe against being called before initialization
        return 0;
