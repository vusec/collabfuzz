...
; RUN: %opt_icount %s                  -icount-abilist=%S/Inputs/abilist.txt -S | FileCheck %s
...
; CHECK-LABEL: @"icount$call_custom_funs_with_callbacks"
define void @call_custom_funs_with_callbacks(i8 (i32, double)* %callback_arg) {
  ;; The callback should have attribute 'nonnull':
  ; CHECK: call signext i32 @__icountw_custom_fun_one_callback(
  ; CHECK: nonnull @"icountt0$custom_fun_one_callback"
  ...
  ; CHECK: call i32 @__icountw_custom_fun_two_callbacks(
  ; CHECK: nonnull @"icountt0$custom_fun_two_callbacks"
  ; CHECK: i64 12345
  ; CHECK: noalias @"icountt2$custom_fun_two_callbacks"
