...
extern "C" void __cond_tracer_create() {
...
// NOLINTNEXTLINE(readability-identifier-naming)
extern "C" void __cond_tracer_trace(std::uint64_t instruction_id,
                                    std::uint64_t total_cases,
                                    std::uint64_t current_case) {
  Expects(current_case < total_cases);

  const std::lock_guard lock(static_vars_mutex);

  // An instrumented constructor may be called before __cond_tracer_create is
  // run.
  if (static_vars != nullptr) {
    static_vars->tracer_->traceCondition(instruction_id, total_cases,
                                         current_case);
  }
}

// NOLINTNEXTLINE(readability-identifier-naming)
extern "C" void __cond_tracer_destroy() {
  const std::lock_guard lock(static_vars_mutex);
  ...
  //  When __cond_tracer_create fails, __cond_tracer_destroy is run anyway.
